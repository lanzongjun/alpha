 ...
 * @author Vincent
 */
class AdMTBillInfoC extends CI_Controller {
    var $_s_view = 'MTBillInfo';
    var $_s_model = 'AdMTBillInfoM';
    
    /**
     * 显示信息
     */
    function index() {
        $data['c_name'] = 'AdMTBillInfoC';
    ...
    function getList() {
        ...
        $o_result = $this->AdMTBillInfoM->_getList($i_page,$i_rows);
    ...
    function uploadInfo() {
        ...
        if (!$result) {
            ...
            $this->load->model($this->_s_model);
            //获得临时表名
            $s_table_name_temp = $this->AdMTBillInfoM->_getTempTableName();
            
            //导入临时表
            $i_rows = $this->AdMTBillInfoM->inputXls($s_table_name_temp, $s_file_path);
    ...
    function loadPreview() {
        $s_tbn = $_GET['tbn'];
        $this->load->model($this->_s_model);
        $o_result = $this->AdMTBillInfoM->_loadTempTable($s_tbn);
        echo json_encode($o_result);
    }
    ...
    function appendData() {        
        $s_tbn = $_POST['tbn'];
        $this->load->model($this->_s_model);
        $b_result = $this->AdMTBillInfoM->doAppendSQL($s_tbn);
    ...
    function deletePreviewData(){
        $s_tbn = $_POST['tbn'];
        $this->load->model($this->_s_model);
        $this->AdMTBillInfoM->_dropTempTable($s_tbn);        
    }
}
