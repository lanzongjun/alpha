 ...
 * @author Vincent
 */
class AdEBBillInfoC extends CI_Controller {
    var $_s_view = 'EBBillInfo';
    var $_s_model = 'AdEBBillInfoM';
    
    /**
     * 显示信息
     */
    function index() {
        $data['c_name'] = 'AdEBBillInfoC';
    ...
    function getList() {
        ...
        $o_result = $this->AdEBBillInfoM->_getList($i_page,$i_rows);
    ...
    function uploadInfo() {
        ...
        if (!$result) {
            ...
            $this->load->model($this->_s_model);
            //获得临时表名
            $s_table_name_temp = $this->AdEBBillInfoM->_getTempTableName();
            
            //导入临时表
            $i_rows = $this->AdEBBillInfoM->inputXls($s_table_name_temp, $s_file_path);
    ...
    function loadPreview() {
        $s_tbn = $_GET['tbn'];
        $this->load->model($this->_s_model);
        $o_result = $this->AdEBBillInfoM->_loadTempTable($s_tbn);
        echo json_encode($o_result);
    }
    ...
    function appendData() {        
        $s_tbn = $_POST['tbn'];
        $this->load->model($this->_s_model);
        $b_result = $this->AdEBBillInfoM->doAppendSQL($s_tbn);
    ...
    function deletePreviewData(){
        $s_tbn = $_POST['tbn'];
        $this->load->model($this->_s_model);
        $this->AdEBBillInfoM->_dropTempTable($s_tbn);        
    }
}
